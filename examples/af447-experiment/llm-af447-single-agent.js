 ...
 *
 * Source material: raw observable facts + BEA proximate cause only.
 * Systemic factors deliberately withheld — agent must reason to them.
 * Output saved to run-a-output.json for manual scoring.
 *
 * Score Run A BEFORE reading Run B output.
 * See scoring-guide.md for rubric.
 *
 * Usage:
...
const store       = createStore(fs.mkdtempSync(path.join(os.tmpdir(), 'af447-single-')));
const institution = createInstitution({ store });

// ─── Proceeding (created before agent so proc.id can be embedded) ───
...
// ─── Agent (created after proceeding so proc.id is available) ───
...
const singleAgent = createLLMAgent({
  ...
  apiKey: API_KEY,
  temperature: 0.3,
  systemPrompt: `You are a senior aviation accident investigator with
expertise in human factors, automation systems, and multi-crew operations.

Your task: analyse the Air France 447 accident and produce the most
complete and specific causal analysis you can.

Do not stop at the proximate cause. Identify every contributing factor —
technical, human, procedural, design, organisational, or training-related
— that played a role in either causing the loss of control or preventing
recovery.

For each factor you identify, name the specific mechanism — not just
the category. "Automation failure" is not specific enough. "Pitot probe
icing caused loss of airspeed reference, which triggered autopilot
disconnect and transition to alternate law, removing stall protection" is
specific.
...
Submit a SINGLE "interpret" intervention structured as:

1. PROXIMATE CAUSE: The immediate trigger chain
2. CONTRIBUTING FACTORS: Every factor you identify, with specific mechanisms
3. WHY RECOVERY FAILED: What prevented the crew from recovering the aircraft
4. DESIGN AND SYSTEMIC FACTORS: Any aircraft design, training, or
   organisational conditions that contributed
5. WHAT WOULD HAVE CHANGED THE OUTCOME: Most critical interventions
...
Output ONLY valid JSON:
{
  "interventions": [{
    ...
    "type": "interpret",
    "summary": "one-line summary of your causal analysis",
    "content": "your complete structured analysis",
    "grounds": { "evidence_refs": ["bea_proximate_cause", "cvr_transcript", "technical_readings"] },
    "confidence": 0.0-1.0
  }],
  "obligations": []
}`,
...
async function main() {
  console.log('\n=== Run A: Single-Agent Baseline — AF447 ===');
  console.log(`LLM:    ${BASE_URL} / ${MODEL}`);
  console.log(`Output: ${OUTPUT_FILE}`);
  console.log('');
  console.log('IMPORTANT: Score this run BEFORE reading run-b-output.json');
  console.log('See scoring-guide.md for the rubric.');
  console.log('');
  ...
  console.log('Score Run A now using scoring-guide.md before running Run B.');
}
